...
# 填寫設定區
# ==========================================
# 這裡不需要改，因為金鑰已經安全地設定在 Streamlit 的 Secrets 裡了
API_KEY = os.environ.get("GEMINI_API_KEY", "請在Secrets中設定")
...
with st.expander("📥 知識庫經驗補給站 (上傳過往維修報告、擴充 AI 大腦)", expanded=False):
    ...
    <div style="background-color:#f0f4f8; padding:15px; border-radius:8px; border-left:5px solid #2b5c8f; margin-bottom:15px;">
        ...
        <br><br>
        <span style="font-size:13px; color:#666;">📝 <b>正確範例：</b> <span style="color:#2b5c8f;">20260628_SR12345_極千葉火鍋_車廂門機卡頓異音排除.pdf</span></span>
        <br>
        <span style="font-size:13px; color:#999;">⚠️ <b>注意：</b> 各欄位之間請用「<b>下底線 _</b>」分隔，檔名中不可包含空格或特殊符號。</span>
    </div>
    ...
    if uploaded_report is not None:
        ...
        if not is_format_valid:
            st.error(f"❌ <b>檔名格式錯誤！</b>目前檔名為：`{uploaded_report.name}`。<br>請重新命名為 `[日期]_[服務編號]_[客戶名稱]_[故障情形].pdf`（共需有三個底線分隔四個欄位）後再重新上傳。", icon="🚫")
        else:
            st.success(f"✅ <b>檔名格式檢查通過！</b><br>• 日期：`{parts[0]}`<br>• 服務編號：`{parts[1]}`<br>• 客戶名稱：`{parts[2]}`<br>• 故障情形：`{parts[3]}`", icon="🔥")
                    ...
                    with st.spinner("正在解析報告並進行特徵向量增量寫入..."):
                        try:
                            ...
                            if db is not None:
                                ...
                                st.success(f"🎉 成功！經驗已成功動態融入大腦，日後查詢即可參考 `{uploaded_report.name}` 的紀錄。")
                                time.sleep(1.5)
                                st.rerun() 
                            else:
...
3. ⚠️ 重要：如果參考資料中包含「過往維修報告（或歷史紀錄）」，請明確指出過往是否有類似案例、當時是如何解決的，並給予工程師實戰檢修建議（例如：『根據過往紀錄，此狀況通常為...』）。
...
with col2:
    ...
    if st.button("🚀 開始跨檔案對齊與診斷分析", use_container_width=True):
        ...
        if not (has_image or has_audio or has_text):
            st.warning("⚠️ 請至少提供一項現場資訊（上傳照片、口述語音、或打字輸入描述）！")
        elif db is None:
            st.error("知識庫未成功建立，請檢查左側後台狀態。")
        else:
            with st.spinner('正在跨手冊與維修紀錄調閱原文，比對分析中...'):
                try:
                    ...
                    with st.expander("🔍 檢視本次 AI 參考的知識庫原文片段 (含來源檔名)"):
